...
def generate_report(path: str, title: str, grammar_text: str, classification: int, steps: List[str], diagram_path: Optional[str] = None):
    ...
    c.drawString(40, y, f"Fecha: {datetime.now()}")
    y -= 20
    c.setFont("Helvetica-Bold", 12)
    c.drawString(40, y, "Gramática / Autómata analizado:")
    y -= 14
    c.setFont("Courier", 9)
    ...
    c.setFont("Helvetica-Bold", 12)
    c.drawString(40, y, f"Clasificación detectada: Tipo {classification}")
    y -= 16
    c.setFont("Helvetica-Bold", 12)
    c.drawString(40, y, "Justificación / Modo explicativo:")
    y -= 14
    c.setFont("Helvetica", 10)
    for s in steps:
    ...
    if diagram_path:
        try:
            c.showPage()
            c.setFont("Helvetica-Bold", 12)
            c.drawString(40, height - 40, "Diagrama")
            ...
            c.showPage()
            c.drawString(40, height - 40, f"No se pudo insertar imagen: {e}")
    c.save()
    return path

...
def split_alternatives(rhs: str) -> List[str]:
    """Divide el lado derecho por |, respetando paréntesis simples si hubiera."""
    parts = []
    buff = []
    bal = 0
    for ch in rhs:
...
def tokenize_rhs(rhs: str) -> List[str]:
    """Tokeniza RHS en terminales/no terminales con reglas simples."""
    tokens = []
    i = 0
    while i < len(rhs):
        ch = rhs[i]
        ...
        if ch.isalpha():
            ...
            tokens.append(ch)
            i += 1
    # Expande para separar letras mayúsculas, etc.
    expanded = []
    for t in tokens:
...
def deduce_symbols(productions: List[Tuple[str, str]]) -> Tuple[Set[str], Set[str]]:
    """Retorna (no terminales, terminales)."""
    NT: Set[str] = set()
    T: Set[str] = set()
